...
policy = PolicyGradient(gen_data, score_up=config["xxx_score_up"], score_down=config["xxx_score_down"])
g_loss_all = []
g_loss_one_epoch = 0
lr = []
...
for epoch in range(config["num_epochs"]):
    ...
    generator.save_model(config['save_addr'] + '/traintime' + str(epoch) + '.pt')
    ...
    discriminator.train()
    loss = discriminator.train_epochs(dis_loader, config["d_epoch"])
    discriminator.save_model(config['save_addr'] + '/traintime' + str(epoch) + '.pt')
