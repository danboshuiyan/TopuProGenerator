...
class Transformer(nn.Module):
    ...
    def generate(self,start_symbol,max_len,temperature=1):
        ...
        with torch.no_grad():
            for i in range(0, max_len):
                ...
                # normalizedlist = scaler.fit_transform(list(projected[0][i].cpu().numpy().reshape(-1,1)))
                # weights = torch.multinomial(torch.tensor(normalizedlist.reshape(1,-1)),1,replacement=True).numpy()
    ...
    def get_prob(self,seq):
        ...
        #     if torch.log(F.softmax(torch.tensor(prob)))[j]<-1000000:
        #         print(prob[j])
        #         print(dec_logits[j])
        # print(torch.tensor(prob))
        # print(len(torch.tensor(prob)))
